...
1차 미분의 근사값 계산을 위해 소벨 Sobel 이란 것을 사용하는데, 이는 X방향 에지와 Y방향 에지 검출을 위한 마스크이다.
...
이와 같이 y에 대한 소벨 마스크도 구한 뒤, 이를 *addWeighted* 함수를 사용해 더해 결과 이미지를 도출한다.<br>(책에서는 *addWeighted* 를 사용했으나, 가중치 없이 그냥 *add* 를 사용해도 무방)
...
이때 좌표 세 점을 지나는 모든 직선에 대해 r과 θ를 구하여 그려보면 3개의 사인곡선이 그려지는데, 이 곡선이 한 점에서 만나는 것을 확인할 수 있다고 한다.<br>
